...
@foreach ([['Because you watched “'.$title.'” — deeper network recommendations', $becauseYouWatched], ['Other movies by actors in “'.$title.'”', $similarFromOtherActors]] as [$heading, $items])
<section class="mt-10"><h2 class="text-2xl font-black uppercase">{{ $heading }}</h2>
@if (count($items))<div class="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">@foreach ($items as $item)<a href="{{ url('/movies/'.rawurlencode($item->title)) }}" class="block border-4 border-black bg-[#ffdf3f] p-5 text-inherit shadow-[6px_6px_0_#171717] hover:translate-x-1 hover:translate-y-1 hover:shadow-none"><h3 class="font-black">{{ $item->title }}</h3><p class="mt-2 text-sm font-bold">@if($item->connectorName && $item->connectorType === 'Actor') Actor: {{ $item->connectorName }} @elseif($item->connectorName && $item->connectorType === 'Director') Director: {{ $item->connectorName }} @else Connected through the movie network @endif</p><p class="mt-1 text-xs font-black uppercase">Distance: {{ $item->distance }} hops</p></a>@endforeach</div>
